            ...
            <div className="w-full max-w-[400px] card-minimal">
                ...
                <div className="space-y-3 mb-8">
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-3 bg-white border border-gray-200 py-3 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 text-sm font-semibold text-gray-700 shadow-sm"
                    >
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    <div>
                        ...
                        <input
                            type="text"
                            className={`input-field ${err.fields.name ? 'border-red-400 ring-4 ring-red-50' : ''}`}
                            placeholder="John Doe"
                            required
                            value={formData.name}
                            onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                        />
                        {err.fields.name && <p className="text-xs text-red-500 mt-1.5 ml-1 font-medium">{err.fields.name}</p>}
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            type="email"
                            className={`input-field ${err.fields.email ? 'border-red-400 ring-4 ring-red-50' : ''}`}
                            placeholder="name@example.com"
                            required
                            value={formData.email}
                            onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                        />
                        {err.fields.email && <p className="text-xs text-red-500 mt-1.5 ml-1 font-medium">{err.fields.email}</p>}
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <input
                                ...
                                className={`input-field pr-12 ${err.fields.password ? 'border-red-400 ring-4 ring-red-50' : ''}`}
                                placeholder="••••••••"
                                required
                ...
                <p className="text-center mt-8 text-sm text-gray-500 font-medium">
                    Already have an account? <Link to="/login" className="text-indigo-500 hover:text-indigo-600 font-bold transition-colors">Sign in</Link>
                </p>
            </div>

            <p className="mt-8 text-xs text-gray-400 font-bold uppercase tracking-widest text-center">
                Uncompromising Quality &bull; Pure Minimal
            </p>
        </div>
    );
};
