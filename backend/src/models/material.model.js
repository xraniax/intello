...
class Material {
  ...
  static async updateStatus(materialId, userId, status, jobId = null) {
    ...
    const startedAtSql =
      normalizedStatus === PROCESSING ? ', started_at = COALESCE(started_at, NOW())' : '';
    const completedAtSql =
      normalizedStatus === COMPLETED || normalizedStatus === FAILED ? ', completed_at = NOW()' : '';
    ...
    const params = jobId
      ? [materialId, normalizedStatus, userId, jobId]
      : [materialId, normalizedStatus, userId];
  ...
  /**
   * Find an active material by title for a specific user and subject.
   */
  static async findActiveByTitle(userId, subjectId, title, type = null) {
    let sql, params;
    if (type) {
      sql =
        'SELECT * FROM materials WHERE user_id = $1 AND (subject_id IS NOT DISTINCT FROM $2) AND LOWER(TRIM(title)) = LOWER(TRIM($3)) AND type = $4 AND deleted_at IS NULL';
      params = [userId, subjectId, title, type];
    } else {
      sql =
        'SELECT * FROM materials WHERE user_id = $1 AND (subject_id IS NOT DISTINCT FROM $2) AND LOWER(TRIM(title)) = LOWER(TRIM($3)) AND deleted_at IS NULL';
      params = [userId, subjectId, title];
    }
    const result = await query(sql, params);
    return result.rows[0];
  }
  ...
  /**
   * Find a material by title regardless of deletion status (Checks Active + Trash).
   */
  static async findByTitle(userId, subjectId, title) {
    const result = await query(
      'SELECT * FROM materials WHERE user_id = $1 AND (subject_id IS NOT DISTINCT FROM $2) AND LOWER(TRIM(title)) = LOWER(TRIM($3))',
  ...
  /**
   * Find a soft-deleted (trashed) material by title.
   * Returns the most recently deleted matching record.
   */
  static async findTrashedByTitle(userId, subjectId, title, type = null) {
    let sql, params;
    if (type) {
      sql =
        'SELECT id, title, type, deleted_at FROM materials WHERE user_id = $1 AND (subject_id IS NOT DISTINCT FROM $2) AND LOWER(TRIM(title)) = LOWER(TRIM($3)) AND type = $4 AND deleted_at IS NOT NULL ORDER BY deleted_at DESC LIMIT 1';
      params = [userId, subjectId, title, type];
    } else {
      sql =
        'SELECT id, title, type, deleted_at FROM materials WHERE user_id = $1 AND (subject_id IS NOT DISTINCT FROM $2) AND LOWER(TRIM(title)) = LOWER(TRIM($3)) AND deleted_at IS NOT NULL ORDER BY deleted_at DESC LIMIT 1';
      params = [userId, subjectId, title];
    }
    const result = await query(sql, params);
